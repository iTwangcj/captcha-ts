...
function letter(n, pos, im, swr, s1, s2) {
    ...
    for (let j = 0, k = t.length; j < k; j++) {
        ...
        if (sk1 >= 200)
            sk1 = sk1 % 200;
        let skew = Math.floor(sw_1.SW[sk1] / 16);
        sk1 += (swr[pos + i - r] & 0x1) + 1;
        if (sk2 >= 200)
            sk2 %= 200;
        let skeWh = Math.floor(sw_1.SW[sk2] / 70);
        sk2 += swr[row] & 0x1;
...
function makeGif(im, gif, style) {
    let r = style === -1 ? random(0, colors_1.colors.length) : style;
    gif.fill(colors_1.colors[r].replace(/\n/g, ''), 0, 13 + 48 + 10 + 1, 'ascii');
    let i = 0;
    let p = 13 + 48 + 10 + 1;
    for (let y = 0; y < 70; y++) {
